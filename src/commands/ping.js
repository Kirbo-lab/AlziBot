...
const { permInvite } = require('../config.json');

module.exports = {
    data: new SlashCommandBuilder()
        .setName('server')
        .setDescription('Replies with the latency of this bot and the API!'),
    async execute(interaction) {
		const { guild } = interaction
		// console.log(guild)

		const { t, language } = guild

		// const { id, name, region, memberCount, channels, commands, emojis, explicitContentFilter, invites, maximumMembers, nsfwLevel, partnered, preferredLocale, premiumSubscriptionCount, premiumTier, roles, shard, shardId, stageInstances, vanityURLCode, vanityURLUses, verificationLevel, verified }

		// console.log(name, region, approximateMemberCount, channels, commands, emojis, explicitContentFilter, )
