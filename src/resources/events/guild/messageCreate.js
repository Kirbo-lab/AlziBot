...
const owner = require('../../misc/configuration/bot.js')
    ...
    execute(message) {
        ...
        const badBot = new MessageActionRow()
            .addComponents(
                new MessageButton()
                    .setStyle('LINK')
                    .setLabel(`Help improve ${bot.application.profile.name} here!`)
        ...
        if (message.content === 'pixel makes the best bots') {
            ...
            console.log(`┌─ `.white + `[${hour}:${minute}]`.brightGreen.bold + ` ` + `(${__filename})`.brightYellow.bold + ` ` + `(LOG)\n`.bold + `└─`.white + ` Replied to message "${message.content}"\n`);
        } else if (message.content === `hi ${bot.application.profile.nameLC}`) {
            ...
            message.channel.send(`<a:loading:887182267254997002> ${bot.application.profile.name} is thinking...`).then(async msg => {
                await wait(2500),
